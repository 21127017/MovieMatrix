...
function App() {
  ...
  const logOut = () => {
    ...
    fetch(`/logout`, requestOptions)
      .catch(error => {
        console.log("erorr logging out", error);
      })
      .finally(() => {
        setJwtToken("");
        toggleRefresh(false);
  ...
  const toggleRefresh = useCallback((status) => {
    ...
    if (status) {
      ...
      setTickInterval(i);
      console.log("setting thick interval to ", i);
    } else {
      console.log("turning off ticking");
      console.log("turing off tickInvetval", tickInterval)
  ...
  return (
    <div className='container'>
      <div className='row'>
        <div className='col'>
          <h1 className='mt-3'>MovieMatrix App!</h1>
        </div>
